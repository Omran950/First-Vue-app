...
function saveSelectedMarkups() {
  if (selectedMarkups.value.length === 0) {
    showStatus('No selected markups to save')
    return
  }

  const rootSvg = markupsCoreButton.svg

  const svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg')
  svg.setAttribute('xmlns', 'http://www.w3.org/2000/svg')
  svg.setAttribute('width', rootSvg.getAttribute('width'))
  svg.setAttribute('height', rootSvg.getAttribute('height'))
  svg.setAttribute('viewBox', rootSvg.getAttribute('viewBox'))

  // Create a group with flip transform
  ...
  selectedMarkups.value.forEach((markup) => {
    if (markup.shape) {
      const cloned = markup.shape.cloneNode(true)
  ...
  if (saveMarkupsToStorage(svgString)) {
    showStatus('Selected markups saved!')
  }
}
...
function loadSavedMarkups() {
  ...
  const doc = parser.parseFromString(saved, 'image/svg+xml')
  const savedSvg = doc.documentElement
  const g = savedSvg.querySelector('g')

  markupsCoreButton.enterEditMode()
  ;(g ? g.childNodes : savedSvg.childNodes).forEach((node) => {
    if (node.nodeType === 1) {
      // Append only shapes, not the <svg> root
      markupsCoreButton.svg.appendChild(node.cloneNode(true))
    }
  })

  markupsCoreButton.leaveEditMode()

  showStatus('Selected markups loaded!')
}
